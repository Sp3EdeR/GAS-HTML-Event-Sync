...
function parseResponses(responses){
  ...
  result.forEach(function(event){
    if (!event.hasProperty('uid')){
      event.updatePropertyWithValue('uid', Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, event.toString()).toString());
    }
...
function createEvent(event, calendarTz){
  ...
  var digest = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, icalEvent.toString()).toString();
  if(calendarEventsHashes.indexOf(digest) >= 0){
  ...
  newEvent.extendedProperties = { private: { HASH : digest, fromGAS : "true", id : icalEvent.uid } };
