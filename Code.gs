...
var sourceCalendars = [                // The urls that you want to parse get events from, along with a regex for parsing them, and their target calendars (list a new row for each mapping)
                                       ...
                                       // The optional "preprocessor" configuration can be used to modify the fetched text before applying the regex.
                                       // The "regex" should contain named groups for: event "dtstart" and "dtend" date (required), event "summary" (required), event "link" (optional), and others.
...
function startSync(){
  ...
  for (var calendar of sourceCalendars){
    ...
    if(addEventsToCalendar || modifyExistingEvents || removeEventsFromCalendar){
      ...
      metadataEventsByCalendar[targetCalendarId] = calendarEvents.slice();
